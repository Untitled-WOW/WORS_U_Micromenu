...
-- function SetupMagicButtons(YOffset, frameName, magicButtons)
    ...
    -- for i, spellData in ipairs(WORS_U_SpellBook.spells) do
        ...
        -- local spellName = select(1, GetSpellInfo(spellID)) -- 2) get spell name
...
-- function SetupPrayerButtons(YOffset, frameName, prayerButtons)
    ...
    -- for i, data in ipairs(WORS_U_PrayBook.prayers) do
        ...
        -- local prayerName = select(1, GetSpellInfo(id)) -- Grab the localized name just once for buff checks & tooltips
		...
		-- -- set buttons Left-click to cast prayer
		-- btn:SetAttribute("type1", "spell") -- Left-click
		-- btn:SetAttribute("spell1", prayerName)
		-- -- set buttons Right-click to cancel prayer
		...
		-- btn:SetScript("OnDragStart", function(self)
			-- if prayerName then
				-- PickupSpell(prayerName)
			-- end
		-- end)		

        -- -- OnUpdate: swap between prayer active and normal icons
