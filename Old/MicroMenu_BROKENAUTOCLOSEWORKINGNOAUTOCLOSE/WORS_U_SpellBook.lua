				...
				if spellData.openInv then
					spellButton:SetScript("PostClick", function()
						...
						watcher:SetScript("OnUpdate", function(self, elapsed)
							...
							-- Wait a little time before checking, to avoid reopening too early
							if self.time > 0.1 and not SpellIsTargeting() then
...
WORS_U_SpellBook.frame:SetPoint("RIGHT")
...
local function OnMagicClick(self)
	...
	if not InCombatLockdown() and WORS_U_MicroMenuSettings.AutoCloseEnabled then
...
SpellbookMicroButton:HookScript("OnEnter", function(self)
    if GameTooltip:IsOwned(self) then
        GameTooltip:AddLine("P or Shift + Click to open WOW Spellbook.", 1, 1, 0, true)
